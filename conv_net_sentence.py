...
def train_conv_net(datasets,
    ...
    for i in range(len(filter_hs)):
        ...
        # conv2_layer = LeNetConvPoolLayer(rng, input=layer0_input,image_shape=(batch_size, 1, img_h, img_w),
        #                         filter_shape=filter_shape, poolsize=pool_size, non_linear=conv_non_linear)
        # layer1_input = np.multiply(conv_layer.output.flatten(2), conv2_layer.output.flatten(2))
...
def make_idx_data_split(train_revs, dev_revs, test_revs, word_idx_map, max_l, k=300, filter_h=5):
    """
    Transforms sentences into a 2-d matrix.
    """
    train, dev, test = [], [], []
...
if __name__=="__main__":
    ...
    if dataset_split=="cv":
        ...
        for i in r:
            ...
            results.append(perf)
        print("test perf: " + str(np.mean(results)))

    else:
        datasets = make_idx_data_split(train_revs, dev_revs, test_revs, word_idx_map, max_l, k=300, filter_h=5)
        perf = train_conv_net(datasets,
                                  U,
                                  lr_decay=0.95,
                                  filter_hs=[3,4,5],
